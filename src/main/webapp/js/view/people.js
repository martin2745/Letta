    ...
    function PeopleView(peopleDao, formContainerId, listContainerId) {
                ...
                if (self.isEditing()) {
                    dao.modifyPerson(person,
                            function (person) {
                                ...
                                $('#person-' + person.id + ' td.login').text(person.login);
                                self.resetForm();
                            },
                            showErrorMessage,
                            self.enableForm
        ...
        this.editPerson = function (id) {
            ...
            if (row !== undefined) {
                ...
                form.find('input[name="login"]').val(row.find('td.login').text());
				...
				<div class="row">\
					...
					<div class="col-sm-2">\
						<input name="surname" type="text" value="" placeholder="Apellido" class="form-control" required/>\
                                     	...
                                     	<div class="col-sm-2">\
						<input id="login" name="login" type="text" value="" placeholder="Login" class="form-control"/>\
					</div>\
                                        <div class="col-sm-2">\
						<input id="password" name="password" type="password" value="" placeholder="contraseña" class="form-control" />\
					</div>\
					<div class="col-sm-2">\
						<input id="role" name="role" value="" placeholder="Rol" class="form-control" />\
					</div>\
					<div class="col-sm-2">\
    ...
    var createPersonRow = function (person) {
        return '<tr id="person-' + person.id + '" class="row">\
			...
			<td class="surname col-sm-3">' + person.surname + '</td>\
                        <td class="login col-sm-3">' + person.login + '</td>\
			<td class="col-sm-3">\
